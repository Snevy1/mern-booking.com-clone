...
export const createPaymentIntent = async(hotelId:string, numberOfNights:string, paymentMethod: 'stripe' | 'mpesa' = 'stripe',
    phoneNumber?: string):Promise<PaymentIntentResponse>=>{
    const response = await fetch(`${API_BASE_URL}/api/hotels/${hotelId}/bookings/payment-intent`, {
        credentials: "include",
        method: "POST",
        body: JSON.stringify({numberOfNights, paymentMethod,
            phoneNumber}),
        headers: {
          "Content-Type": "application/json",
        }
